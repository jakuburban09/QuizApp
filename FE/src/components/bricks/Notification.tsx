...
const Notification: FC<NotificationProps> = ({
  ...
  return (
    <div
      ...
      )} text-white flex justify-between w-full rounded-xl
    `)}
    >
      <AnswerIndicator
        type={typeOfAnswerIndication}
        backgroundColor={backgoundIndicatorSwitch}
        isCorrect={false}
      />
      ...
      {showDetail && (
        <button onClick={onDetail} className="mr-2 text-sm underline">
          Detail
        </button>
      )}
      <div className="m-2 flex items-center">
        <Button
          ...
          color={Color.Gray25}
          icon={{ iconName: "XLg", color: "black", width: 28, height: 28 }}
          rounded
        >
          {" "}
        </Button>
